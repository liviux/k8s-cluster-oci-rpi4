...
resource "oci_network_load_balancer_backend" "k3s_kubeapi_backend_extra_server_node" {
    count                    = var.expose_kubeapi && var.k3s_extra_server_node ? 1 : 0
    backend_set_name         = oci_network_load_balancer_backend_set.k3s_kubeapi_backend_set[0].name
    network_load_balancer_id = oci_network_load_balancer_network_load_balancer.k3s_public_lb.id
    name                     = format("%s:%s", oci_core_instance.k3s_extra_server_node[count.index].id, var.kube_api_port)
    port                     = var.kube_api_port
    target_id                = oci_core_instance.k3s_extra_server_node[count.index].id
  }